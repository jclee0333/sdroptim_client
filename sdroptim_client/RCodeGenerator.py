...
''' + doVectorization(gui_params) 
    ...
    train_test_split = '''##* Train/Test Dataset Split and Scaling
'''
...
'''
    ...
    scaling_iteration = "scaled_all_data <- all_data\n"
    #scaling_iteration = ""
    for column_index in gui_params['datatype_of_columns']:
        if gui_params['datatype_of_columns'][column_index] == 'Numeric':
            scaling_iteration = scaling_iteration + "scaled_all_data$"+gui_params['whole_columns_index_and_name'][column_index]+" <- scale(all_data$"+gui_params['whole_columns_index_and_name'][column_index]+")\n"
    #for item in input_cols:
    #    scaling_iteration = scaling_iteration + "scaled_all_data$"+item+" <- scale(all_data$"+item+")\n"

    split = "train_data = scaled_all_data[1:split_num, ]\ntest_data = scaled_all_data[(split_num+1):nrow(scaled_all_data), ]\n"

    return data_loading +pre_processing_imputation +pre_processing_vectorization + "\n"+ train_test_split + scaling_iteration + split
...
def getHyperParametersandClosing(gui_params):
    ...
    if gui_params['hparams']:	# if it exists
        ...
        for hparam in gui_params['hparams']:
            ...
            elif gui_params['algorithm'] == 'DL_old':
                if hparam == 'cv':
                    annotate_list.append("k-fold cross-validation")
...
def getDeepLearning_old(gui_params):
    # getting column names
    input_cols, output_col, input_index_list, output_index = getInputOutputColumnNames(gui_params)
...
def getDeepLearning(gui_params):
    ...
    # getting column names
    input_cols, output_col, input_index_list, output_index = getInputOutputColumnNames(gui_params)
    results = "## Extracting data frame\nlibrary(keras)\n"
    removed_list_that_is_categorical_inputs = []
    for column_index in gui_params['datatype_of_columns']:
        temp = gui_params['whole_columns_index_and_name'][column_index]
        if temp in input_cols:
            if gui_params['datatype_of_columns'][column_index] == 'Numeric':
                pass
            else:
                removed_list_that_is_categorical_inputs.append(temp)
                input_cols.remove(temp)
    results = results +'x_train <- as.matrix(train_data[,c('+str(input_cols)[1:-1]+')]' + ")\n"
    results = results +'x_test <- as.matrix(test_data[,c('+str(input_cols)[1:-1]+')]'+ ")\n"

    if removed_list_that_is_categorical_inputs: #if exists
        results = results + "\n# adding categorical inputs\n"
    for item in removed_list_that_is_categorical_inputs:
        results = results + 'x_cate <- to_categorical(as.numeric(all_data$' + item + '))[,c(-1)]\n'
        results = results + 'x_train <- cbind(x_train, x_cate[1:split_num,])\nx_test <- cbind(x_test, x_cate[(split_num+1):nrow(all_data),])\n\n'
    if gui_params['task'] == 'Regression':
        results = results + 'y_train <- as.matrix(train_data$'+ output_col + ')\n'
        results = results + 'y_test <- as.matrix(test_data$' + output_col + ')\n'
    elif gui_params['task'] == 'Classification':
        results = results + "n_levels = length(levels(all_data$" + output_col + "))\n"
        results = results + 'y_cate <- to_categorical(as.numeric(all_data$'+output_col + '))[,c(-1)]\n'
        results = results + 'y_train <- y_cate[1:split_num,]\ny_test <- y_cate[(split_num+1):nrow(all_data), ]\n'
    hidden_net_params=""
    cv_num = -1
    results = results + "\n##* Hyperparameters for deep learning\ninput_dim = ncol(x_train)\t# number of features\n" 
    if gui_params['hparams']:   # if it exists
    ...
    if cv_num > 0:
        results = results + "##* cross validation score\nlibrary(caret)\nkfold <- createFolds(train_data$"+output_col+", k = "+str(cv_num)+")\n"
        results = results + '''kfold_cv <- sapply(kfold, function (idx) {
    x_train.train.i <- x_train[-idx, ]
    y_train.train.i <- y_train[-idx, ]
    x_train.test.i <- x_train[idx, ]
    y_train.test.i <- y_train[idx, ]
    history <- model %>% fit(x_train.train.i, y_train.train.i, epochs = epochs, batch_size = batch_size)
    result <- model %>% evaluate(x_train.test.i, y_train.test.i)
    return(result)
})
print(kfold_cv)
'''
    ...
    if gui_params['task'] == 'Regression':
        results = results + "predicted <- model %>% predict(x_test)\n"
    elif gui_params['task'] == 'Classification':
        results = results + '## The keras predict function gives probabilities rather than categorical outputs, so we modify to calculate confusion matrix by using caret library.\npredicted_prob <- model %>% predict(x_test)\nnames_from_prob = levels(all_data$'+output_col+')\n'+'predicted=as.factor(apply(predicted_prob, 1, which.max))\nlevels(predicted) = names_from_prob\n'
...
def getDeepLearningHiddenNetwork(gui_params, hidden_net_params):
    ...
    if gui_params['task'] == 'Regression':
        ...
        activation = 'tanh'
        metrics = "'accuracy'"
    middle_1 = "layer_dense(units = " + hidden_net_params[0] + ", input_shape = input_dim, activation = '"+activation+"') %>%\n"
    for i in range(1,len(hidden_net_params)):
        middle_2 = middle_2 + getDeepLearningDenseLayer(hidden_net_params[i], activation, "middle_2")
    if gui_params['task'] == 'Regression':
        middle_3 = getDeepLearningDenseLayer(1, "", "middle_3") 
    elif gui_params['task'] == 'Classification':
        middle_3 = getDeepLearningDenseLayer("n_levels", "softmax", "middle_3")
    #bottom = "    model.compile(loss = loss, optimizer = optimizer, metrics = ["+metrics+"])\n    model.summary()\n    return model\n"
    bottom = "model %>%  compile(loss = loss, optimizer = optimizer, metrics = c(" + metrics + "))\n"
    return top + middle_1 + middle_2 + middle_3 + "\n" + bottom + "\n## Model summary\nsummary(model)\n"

def getDeepLearningDenseLayer(num, act, target):
    results = ""
    if act == "":
        results = "layer_dense("+str(num)+")"
    else:
        results = "layer_dense("+str(num)+", activation = '" + str(act) + "')"
    if target == 'middle_2':
        results = results + " %>%\n"
    else:
        results = results + "\n"
...
#                    results = results + '\nperf_r2 = h2o.r2(model, xval=FALSE)\t#using xval=TRUE when retrieving the cross-validation R2\n'
                else:	# in the cases of SVM, RF, BT, LR, DL
...
'''
        ...
        # mandantories - nfolds, epochs, hidden
        defaultParameters = '''
#- input_shape // optional shape list. only to be specified if include_top is FALSE
#- epochs = 10 // How many times the dataset should be iterated (streamed).
...
#- hidden = "20,20" // Hidden layer sizes. It can be modified in the learning network design.
...
'''
        referenceAddress = "#- https://cloud.r-project.org/web/packages/keras/keras.pdf\n"
